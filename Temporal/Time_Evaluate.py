...
import gzip
###########################预测的数据#####################################
df2 = pd.read_csv(r'Time_C_北京市station_换电数据_flow_2_del_normalized.csv')
...
pred_data = pred_data[7:14, :24, :24]    #第一个参数：天；第二个参数：城市区域；第三个参数：小时

###########################真实的数据#####################################

# 读取 CSV 文件
df = pd.read_csv(r'Time_C_北京市station_换电数据_flow_2_del_normalized.csv')
...
reshaped_data = reshaped_data[7:14, :24, :24]
